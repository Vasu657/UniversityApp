...
const AnimatedTaskCard = ({ item, index, onUpdateTask }) => {
  ...
  const renderLinks = (text) => {
    ...
    return parts.map((part, index) => {
      ...
      return <Text key={index} style={styles.taskDetail}>{part}</Text>;
    });
  };
      ...
      <Animated.View style={[styles.taskCardContainer, animatedStyle]}>
        <Card style={[styles.taskCard, { borderLeftColor: dueDateStatus.color }]}>
          <Card.Content>
            ...
            <Text style={styles.taskDetail}>Assigned by: {item.assigned_by}</Text>
            ...
            {item.notes && (
              <View style={styles.taskDetail}>
                <Text style={styles.taskDetail}>Notes: </Text>
                {renderLinks(item.notes)}
              </View>
            )}
            {item.link && (
              <View style={styles.taskDetail}>
                <Text style={styles.taskDetail}>Link: </Text>
                <Text
                  style={[styles.taskDetail, { color: theme.colors.primary, textDecorationLine: 'underline' }]}
